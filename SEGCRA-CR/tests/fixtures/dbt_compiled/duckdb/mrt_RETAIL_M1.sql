...
, T_TXN_AGG2 AS (
    ...
    (
        direction_flag = '0'
        AND amount > 1000
        AND op_code IN ('OP01','OP02','OP03')
    )
 THEN 1 ELSE 0 END) AS inbound_cnt
        , ISNULL(
            SUM(CASE WHEN 
 ...
 THEN amount ELSE 0 END)
    ...
    (
        direction_flag = '1'
        AND op_code IN ('OP11','OP12')
    )
 THEN 1 ELSE 0 END) AS kiosk_outbound_cnt
        , ISNULL(
            SUM(CASE WHEN 
...
WHERE NOT EXISTS (
    SELECT 1
    FROM "SAMPLE_DW"."dbo"."retail_m1_earlyjob" e
    WHERE e.account_id = f.account_id
    AND e.TABLE_DATE = DATEADD(DAY, -1, '2026-02-01')
)
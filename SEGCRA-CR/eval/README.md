...
> ⚠️ **可重現性驗證做了,結果是「部分可重現」,不是完全一致**(2026-09-17)。把 `mr_406`
> 連跑兩次比對:**驅動這張表 pass/fail 的東西**(`_golden` 斷言結果、`spec_exec` 的數字
> ——測資案例數、覆蓋缺口數、`passed` 判定)兩次完全一樣,所以這張表當回歸基準線是可信的。
> 但**完整審查報告的文字內容不可重現**——兩次的決策說明措辭不同,finding 數量也不同
> (第 2 次還把同一條 finding 重複輸出了兩次,是額外挖到的獨立 bug)。細節與 Rlong 給的
> 比對腳本本身的問題,見 [FINDINGS.md](FINDINGS.md)「發現十三」的後續更新。

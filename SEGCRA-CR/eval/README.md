...
`mr_401` 正是專案交接文件點名的情境(規格寫「大於」、程式寫 `>=`)。
...
| **bare LLM 對照組** | **未跑** —— 專案交接文件對階段 0 的收尾要求 |

最後一項最重要。交接文件明訂:

> 建 baseline 量尺:同一批 MR 跑 **bare LLM vs 這包**…目標:比 bare LLM 好、且不退步

`demo.py --baseline` 即裸模型模式(無 skills / 記憶 / 預掃 / 執行驗證)。
**沒有對照組,「我們比較好」只是宣稱。**

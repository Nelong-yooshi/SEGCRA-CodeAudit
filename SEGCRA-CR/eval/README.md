...
> ⚠️ **更正(2026-09-23)—— seed 有生效,而且量測模式下這條管線是可重現的**:
> 更早一版寫「部分可重現」,後來查 `seed` 為什麼看起來從未生效時,曾判定是模型端點
> 接受但忽略 `seed`——**那個根因判定是錯的,已收回**:重做對照實驗,同 seed 兩次結果
> 逐字相同、拿掉 seed 的對照組兩次不同,seed 是有生效的。
>
> 接著把 `mr_406` 在量測模式(`temperature=0` + `seed=42`)下連跑 5 次:
> **五次的案例數、缺口數、決策完全相同**,耗時也落在 658~679 秒之間。
> 而同一個 case 不帶 seed 跑兩次則是 19 案例/0 缺口 vs 10 案例/5 缺口。
> 把每次實際送出的 prompt 存下來比對後確認:不帶 seed 那兩次,**測資生成那一次呼叫的
> prompt 兩次 md5 完全相同**,輸出卻差了快一倍——所以飄動來自**取樣**,
> 不是先前猜的「沙盒資料庫名稱、時間戳記混進 prompt」(查過,那些根本沒進 prompt)。
>
> **回歸比較仍然跑多次看分布**,但理由改了:不是「沒有可重現的機制」,而是
> **其餘 case 還沒量過**——目前只有 `mr_406` 有 5 次的數據。
> 細節見 [FINDINGS.md](FINDINGS.md)「發現十三」的後續更新與「附錄:`seed` 根因追查」。
...
- **意外失敗**:非設計預期、值得跟進的真問題——`mr_204` 見 [FINDINGS.md](FINDINGS.md)
  「發現十二」,`mr_405` 見「發現十四」。
- 這是排除環境雜訊(卡死、單次生成超過等待上限被中途切斷的偶發錯誤)之後的最終結果,
  完整過程見 `FINDINGS.md`「實測結果 四部曲」。這裡只給速覽。
- ⚠️ 模型端點前面那層的等待上限已於 2026-09-19 由 15 分鐘調成 1 小時,`mr_403`、
  `mr_405`、`mr_406`、`mr_408` 這幾個當初被判定卡死或逾時而放棄的 case,建議重跑一次
  再下結論——上表這幾個 case 的結果可能只是生成太久被切斷,不是真的沒通過。
...
模型端點是經一層轉發接入的,那層轉發斷開時本機 listener 不會察覺,
請求等同送進黑洞。
...
2. **在伺服器上以 `nohup` 執行**。模型呼叫直接打伺服器本機的模型端點,不經 SSH 轉發;
   SSH 斷線不影響評測(本輪已驗證有效)。

...
> ⚠️ **更正(PR #9 review 回覆)—— 往後仍當作不能重現,但 seed 其實有生效**:更早一版寫
> 「部分可重現」,後來查 `seed` 為什麼看起來從未生效時,曾判定是模型端點前面那層代理的
> 轉發路徑會接受但忽略 `seed`——**這個根因判定後來被證明是錯的,已收回**:在模型端點上
> 重做對照實驗,同 seed 兩次結果逐字相同,seed 是有生效的。`mr_406` 觀察到的兩次結果不同,
> 較可能是兩次送進模型的 prompt 內容本身就不一樣(例如沙盒隨機產生的資料庫名稱、時間戳記
> 混進了 prompt),而不是取樣隨機性或 seed 失效——這件事還沒查清楚。在查清楚之前,
> **回歸比較仍統一當作不能重現**,跑多次看分布(而非單次比對),這個保守做法本身沒有錯。
> 根因細節見 [FINDINGS.md](FINDINGS.md)「發現十三」的後續更新與「附錄:`seed` 根因追查」。

...
> ⚠️ **更正(2026-09-18)—— 找到根因,結論改為「當作不能重現」**:上一版寫「部分可重現」
> (`spec_exec` 的數字結果一致,報告文字不一致),那是根據 `mr_406` 背靠背連跑兩次剛好拿到
> 一樣的數字下的結論。後續查 `seed` 為什麼從未在 Ollama 端生效時發現:`:11435` **不是
> Ollama,是一層叫 `ollama-gate` 的 Python 代理**(`Server: ollama-gate Python/3.9.6`),
> 管線走的 `/v1` 路徑會**接受但忽略** `seed`(原生 `/api/chat` 有效,已實測對照)。
> **也就是說我們沒有任何取得可重現結果的機制**——`mr_406` 那兩次數字一樣,是貪婪解碼
> (`temperature=0`)那次剛好沒飄,不是 `seed` 真的鎖住了什麼。「部分可重現」這個說法會讓
> 人以為系統有這個性質,但它沒有,比簡化更容易誤導。**往後一律當作不能重現**,回歸比較
> 改採多次跑、看分布(而非單次比對)。根因細節見 [FINDINGS.md](FINDINGS.md)「發現十三」
> 的後續更新與「附錄:`seed` 根因追查」。
...
另外,LLM 層有隨機性。回歸比較(`eval/run_eval.py` 平常跑法)固定 `temperature: 0`,但
**`seed` 在目前的基礎設施上從未生效**(端點 `ollama-gate` 的 `/v1` 路徑會接受但忽略它,
細節見 `FINDINGS.md`「發現十三」後續與「附錄:`seed` 根因追查」)——所以**當作不能重現**,
不能只跑一次就當結論,要跑多次看分布。出能力報告(要看模型真實分布)則保留隨機性、多次
取樣取平均,做法見 `config/models.yaml` 開頭的註解。
